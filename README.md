...
- 길찾기의 기준점이 되는 **집 주소**, **외출 준비 시간**, 메인페이지에 보일 **달력**을 선택할 수 있는 페이지입니다.
- 설정페이지에 적용한 값은 메인페이지와 상세 페이지에 반영됩니다.
...
| 다음 postcode 서비스를 이용하여 주소 설정 | 외출 준비 시간은 일정의 알림 제공 시간에 반영  | 구글 캘린더에 저장된 캘린더 리스트들 중 토글버튼을 통하여 선택 | 
